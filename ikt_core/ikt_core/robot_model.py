...
try:
    # Tool-frame (R2) URDF augmentation. Vendored in this package's urdf_utils
    # so the library is self-contained when pip-installed; fall back to the
    # toolkit's ikt_common copy for older installs.
    from .urdf_utils import augment_urdf
...
class RobotModel:
    ...
    def supporting_joints(self, frame: str) -> List[str]:
        ...
        frame_obj = self.model.frames[fid]
        # Pinocchio renamed Frame.parent -> Frame.parentJoint; prefer the new
        # name and fall back for older Pinocchio builds.
        parent_joint = getattr(frame_obj, "parentJoint", None)
        if parent_joint is None:
            parent_joint = frame_obj.parent
